...
public class EntityHandlerImpl implements EntityHandler {
   ...
   public double getStepHeight(Entity entity) {
      return (double)EntityUtils.nms(entity).dG();
   }
   ...
   public void movePassenger(Entity entity, double x, double y, double z) {
      ...
      double seatY = y + (double)nmsEntity.k(this.dummyArmorStand);
   ...
   public MoveController wrapMoveController(Entity entity, Supplier<MoveController> def) {
      ...
      if (var4 instanceof EntityInsentient) {
         EntityInsentient mob = (EntityInsentient)var4;
         MoveControlWrapper controller = new MoveControlWrapper(mob, mob.K());
         ...
         try {
            ...
            if (oldNav instanceof NavigationSpider) {
               ...
               if (!(oldNav instanceof AmphibiousPathNavigation)) {
                  EntityTypes var10000 = mob.ai();
                  TLogger.warn("Failed to create custom navigation for " + var10000 + ": " + mob.cw());
   ...
   public void spawnDynamicHitbox(DynamicHitbox hitbox) {
      ...
      final PacketPlayOutSpawnEntity hitboxSpawn = new PacketPlayOutSpawnEntity(DynamicHitbox.getHitboxId(), DynamicHitbox.getHitboxUUID(), location.getX(), location.getY() - 0.5202D, location.getZ(), 0.0F, 0.0F, EntityTypes.aM, 0, Vec3D.b, 0.0D);
   ...
   public TrackedEntity wrapTrackedEntity(Entity entity) {
      ...
      Map<Integer, EntityTracker> map = level.l().a.K;
   ...
   private static boolean isVisible(CraftWorld world, Vec3D startPos, Vec3D endPos) {
      MovingObjectPositionBlock nmsHitResult = usePaperClipMethod ? world.getHandle().a(new OcclusionClipContext(startPos, endPos)) : world.getHandle().clip(new OcclusionClipContext(startPos, endPos), (Predicate)null);
