...
public class ConfigureHitboxMechanic implements ITargetedEntitySkill {
   ...
   public ConfigureHitboxMechanic(MythicLineConfig mlc) {
      ...
      this.shouldPass = mlc.getPlaceholderDouble(new String[]{"pass"}, 1.0D, new String[0]);
   }
      ...
      if (model == null) {
         ...
         model.getModel(modelId).ifPresent((activeModel) -> {
            ...
            activeModel.getBone(partId).flatMap((modelBone) -> {
               ...
               ((SubHitbox)subHitbox).setDamageMultiplier((float)this.shouldPass.get(meta, target));
            });
         });
